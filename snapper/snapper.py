"""
BigSnapper Automated Trading Engine v6.0
Flask webhook → Dhan API → NSE

Signal protocol (Pine R3.3):
  ENTRY : BUY  SYMBOL price SL=xxx TP=xxx
  ENTRY : SELL SYMBOL price SL=xxx TP=xxx
  EXIT  : EXIT SYMBOL price TPHIT|SLHIT|MAFLIP

Pine handles: signals, SL, TP, time window, filters
Python handles: receive alert, place order, track position
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f"logs/snapper_{datetime.now().strftime('%Y%m%d')}.log", encoding="utf-8"
        ),
...
def state_file():
    return f"data/snapper_{ist_now().strftime('%Y%m%d')}.json"
...
def parse_signal(raw: str):
    ...
    try:
        ...
        if action not in ("BUY", "SELL", "EXIT") or len(parts) < 3:
            return None, None, None, None, None, None

        symbol = parts[1].upper().split(":")[-1]  # strip NSE: prefix
        ...
        sl, tp, exit_type = None, None, None

        if action == "EXIT":
            exit_type = parts[3].upper() if len(parts) >= 4 else "MANUAL"
        else:
            for part in parts[3:]:
        ...
        logger.info(
            f"📥 action={action} symbol={symbol} price={price} sl={sl} tp={tp} exit={exit_type}"
        )
        return action, symbol, price, sl, tp, exit_type
...
def health():
    return jsonify(
        {"status": "ok", "time": ist_str(), "dhan": dhan is not None, "version": "v6.0"}
    )
...
def test_parse():
    """Dry-run — parse a signal without placing any order."""
...
if __name__ == "__main__":
    logger.info("=" * 55)
    logger.info("  BigSnapper Trading Engine v6.0")
    logger.info("=" * 55)
    load_config()
    init_dhan()
    load_security_ids()
