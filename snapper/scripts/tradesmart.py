...
def place_order(
    ...
    signal_price: float = None, is_exit: bool = False
) -> dict | None:
    """
    All orders → MARKET immediately.
    Price is ignored — Dhan fills at best available price on NSE.
    """
    try:
        txn = dhan.BUY if action == "BUY" else dhan.SELL
        logger.info(f"📤 {action} {symbol} × {quantity} @ MARKET")
...
def parse_signal(raw: str):
    """
    TradeSmart 2-alert format:

    A1 — Entry alert (LONG or SHORT):
      Message format: SYMBOL|price
      Python checks current signalState from Pine to know direction.
      Since Pine only fires A1 on newLong or newShort, we track
      direction via position state — if no position open → determine
      from context. We embed direction in message for clarity:
        LONG|SYMBOL|price   → BUY
        SHORT|SYMBOL|price  → SELL

    A2 — Exit alert (TP or SL):
      Message format: SYMBOL|price
      Python just squares off whatever position is open.
    ...
    try:
        ...
        if len(parts) < 2:
            logger.warning(f"⚠️  Unknown signal format: {raw}")
            return None, None, None, None, None, None

        first = parts[0].upper()

        # ── A1 Entry: LONG|SYMBOL|price or SHORT|SYMBOL|price ─
        if first in ("LONG", "SHORT"):
            ...
            logger.info(f"📥 {first} entry | {symbol} @ {price}")
            return action, symbol, price, None, None, None

        # ── A2 Exit: SYMBOL|price (no direction prefix) ───────
        # Symbol won't be LONG/SHORT so it falls here
        else:
            symbol = parts[0].upper().split(":")[-1]
            price  = float(parts[1]) if len(parts) > 1 else None
            logger.info(f"📥 EXIT | {symbol} @ {price}")
            return "EXIT", symbol, price, None, None, "EXIT"
...
def webhook():
    ...
    with position_lock:
        ...
        if action == "EXIT":
            ...
            result = place_order(close_side, symbol, pos["quantity"], sec_id, signal_price=None, is_exit=True)
        ...
        # ── ENTRY ─────────────────────────────────────────────
            ...
            # Step 1 — check if opposite position already open → square off first
            existing = positions.get(symbol)

            if existing:
                existing_side = existing["side"]
                is_opposite   = (action == "BUY" and existing_side == "SELL") or                                 (action == "SELL" and existing_side == "BUY")
                is_same       = existing_side == action

                if is_same:
                    # Same direction already open — ignore
                    logger.info(f"⏭️  {symbol} already {action} — signal ignored")
                    return jsonify({"status": "ignored", "reason": f"Already {action} on {symbol}"})

                if is_opposite:
                    # Opposite position open — square off first
                    close_side = "SELL" if existing_side == "BUY" else "BUY"
                    logger.info(f"🔄 Squaring off {existing_side} before entering {action} on {symbol}")
                    sq_result = place_order(close_side, symbol, existing["quantity"], sec_id,
                                            signal_price=None, is_exit=True)
                    if sq_result:
                        # Calculate P&L on square off
                        pnl = 0.0
                        if existing.get("entry_price") and price:
                            pnl = ((price - existing["entry_price"]) * existing["quantity"]
                                   if existing_side == "BUY"
                                   else (existing["entry_price"] - price) * existing["quantity"])
                            total_pnl += pnl
                        ...
                        del positions[symbol]
                        logger.info(f"✅ Squared off {existing_side} {symbol} | P&L ₹{pnl:.2f}")
                    else:
                        logger.error(f"❌ Square off failed for {symbol} — new entry blocked")
                        return jsonify({"error": "Square off failed — entry blocked"}), 500

            # Step 2 — check max positions (only if no existing position was just closed)
            if symbol not in positions and max_positions_reached():
                return jsonify({
                    "status": "skipped",
            ...
            result = place_order(action, symbol, quantity, sec_id, signal_price=price, is_exit=False)
