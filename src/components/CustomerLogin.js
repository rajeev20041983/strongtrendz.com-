...
const CustomerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // which would trigger a Lambda function to authenticate customers
    ...
    <div className="py-16 bg-gray-100 min-h-screen" style={{backgroundImage: 'url(/stock-bg.jpg)', backgroundSize: 'cover', backgroundPosition: 'center'}}>
      <div className="container">
        <div className="max-w-md mx-auto bg-white rounded-lg shadow-lg p-8 relative z-10 bg-opacity-95">
          <h2 className="text-3xl text-primary font-bold mb-6 text-center">Customer Login</h2>
          
          {error && <div className="bg-red-100 text-red-700 p-3 rounded mb-4">{error}</div>}
          
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block text-gray-700 mb-2" htmlFor="email">Email / User ID</label>
              <input
                type="text"
                id="email"
